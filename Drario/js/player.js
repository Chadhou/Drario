(function() {
	...
	var Player = Mario.Player = function(pos) {
		...
		// default walk speed and acceleration (slower)
		this.maxSpeed = 3.0;
		this.moveAcc = 0.12;
	...
	Player.prototype.jump = function() {
	 		// If currently adjusting jump height, continue that
	 		if (this.jumping) {
	 			this.jumping -= 1;
	 			return;
	 		}
	 		...
	 		// Initial ground jump
	 		if (this.standing && this.canJump) {
	 			this.jumping = 28;
	 			this.canJump = false;
	 			this.standing = false;
	 			this.vel[1] = -9;
	 			// allow one additional mid-air jump
	 			this.jumpsRemaining = 1;
	 			if (this.power === 0) {
	 		...
	 		// Double jump: allowed only when airborne and extra jump remains
	 		if (!this.standing && this.jumpsRemaining > 0) {
	 			// give a shorter extra boost
	 			this.jumping = 12;
	 			this.vel[1] = -8;
	 			this.jumpsRemaining -= 1;
	 			// play jump sound
  ...
  Player.prototype.setAnimation = function() {
	...
	Player.prototype.update = function(dt, vX) {
		...
		// Only decrement the numeric invincibility counter when not toggled on
		if (this.invincibility && !this.invincibilityToggle) {
		...
		if (Math.abs(this.vel[0]) > this.maxSpeed) {
			// light drag for controlled speed
			this.vel[0] -= 0.002 * this.vel[0] / Math.abs(this.vel[0]);
			this.acc[0] = 0;
		}
		...
		if (this.dying){
			...
			// If flying, apply upward acceleration (negate gravity)
			if (this.flying) {
				this.acc[1] = -0.35; // upward thrust while holding P
				// clamp upward velocity so it doesn't get too large
				if (this.vel[1] < -10) this.vel[1] = -10;
			} else {
				this.acc[1] = 0.25; // normal gravity
			}
			if (this.pos[1] > 240) {
				this.die();
			}
		...
		if (this.exiting) {
			...
			if (this.pos[0] >= this.targetPos[0]) {
				...
				window.setTimeout(function() {
					...
					vX = 0;
					vY = 0;
					// Load next level with a small delay
					window.setTimeout(function() {
						if (level.loader) {
							level.loader();
						}
